...
Codex discovery relies on the installed skill metadata, not `commands/`. Keep `skills/agentic-code-reviewer/SKILL.md` discoverable for phrases like `run code-review on this repo`, `run the code-reviewer skill`, and `run the agentic-code-reviewer skill`; these are trigger phrases only and do not rename the skill path or package identity.
