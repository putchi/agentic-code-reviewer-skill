...
<head>
  ...
  <style>
    body { margin: 0; padding: 0; height: 100vh; display: flex; flex-direction: column; overflow: hidden; background: var(--vscode-editor-background); color: var(--vscode-editor-foreground); }
    iframe { flex: 1; width: 100%; border: none; }
  </style>
</head>
<body>
  <iframe id="acr-frame" src="${url}"></iframe>
  ${themeScript}
  <script>
    (function() {
      var ready = false;
      ...
      window.addEventListener("message", function(e) {
        if (e.data === "acr-ready") { ready = true; }
      });
      setTimeout(function() {
        if (!ready && reloads < 1) {
          reloads++;
          var f = document.getElementById("acr-frame");
          if (f) { f.src = f.src; }
        }
      }, 3000);
    })();
