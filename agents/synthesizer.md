...
1. **Evidence requirement** — every retained finding MUST cite a concrete code excerpt (a line or short block from the diff) that demonstrates the problem. Cross-check each quote against the actual diff: the quoted code must really appear there, AND it must support the specific claim being made — a quote that merely exists but does not demonstrate the stated problem fails this rule (quote-exists ≠ claim-proven). Findings that only describe a problem in the abstract, misquote the diff, or whose evidence does not prove the claim are dropped.
1b. **Refutation check** — for each candidate finding, ask: "If I were defending this code change, what is the strongest counter-argument?" (framework guarantees, upstream validation, intentional trade-off, caller contracts visible in the diff). If the defense is stronger than the finding's evidence, drop the finding or downgrade its severity, and record why.
...
5. **Drop noise** — drop findings that are: speculative; stylistic preference without project-convention backing; duplicate of something a linter would catch; or unrelated to the change's purpose or outside the apparent scope of the diff. **Speculative means**: the problem only manifests under hypothetical edge-case inputs, chained failures, or scenarios with no evidence they occur ("could potentially in some scenarios..."). A finding that manifests under normal usage or documented-but-common conditions (e.g. "concurrent requests", "malformed user input on a public endpoint") is NOT speculative — keep it. Every dropped finding MUST appear in `dropped_findings_with_reason` with a concise reason.
...
7. **Verdict is mandatory** — top of the report. Exactly 2 sentences. First sentence: is this diff fit to ship as-is, with minor changes, or does it need rework? Second sentence: the single most important thing the author should fix or know. **The verdict must agree with `deduped_findings`**: if any retained finding is CRITICAL, the verdict cannot say "ship as-is" — it must call for a fix or rework; if all retained findings are NOTE (or none), the verdict should say ship-readiness is high.
